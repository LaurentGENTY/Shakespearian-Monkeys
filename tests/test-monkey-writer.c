...
char test_name[] = "test-monkey-writer";

/*
* Here we test the case where we added a new writer monkey, and we check if it
* had been successfully added.
*/
int test_create_writer(struct monkey *monkeys[])
{
  return (monkeys[WRITER]->role == WRITER && monkeys[WRITER]->activity == 0 && monkeys[WRITER]->queue != NULL);
}

/*
* Here we test the case where the writer monkey tries to get the first word in
* the writer queue and write random words from it (and at the end, we
* check that he is still active and alive).
*/
int test_work_writer(struct monkey **monkeys,
	 ...
	 struct poolQueues *poolQueues)
{
  ...
  struct cell *c = pop(queueWriter);
  return (strcmp(c->word, "a") == 0
          && monkeys[WRITER]->activity == 1
          && len == 2);
}
...
int main()
{
  ...
  int t_create_writer = 0, t_work_writer = 0;
  ...
  if(init_read())
  {
    ...
    t_create_writer = test_create_writer(monkeys);
    log_test(t_create_writer, "test_create_writer");
    ...
    len = (monkeys[READER]->activity == 1)+(monkeys[STATISTICIAN]->activity == 1);
    t_work_writer = test_work_writer(monkeys, &queueReader, &queueStats, &queueWriter, &pool, &poolQueues);
    log_test(t_work_writer, "test_work_writer (only one word)");
  ...
  if(t_create_writer && t_work_writer)
  {
    return EXIT_SUCCESS;
  }
  else
